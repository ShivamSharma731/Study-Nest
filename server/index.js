...
app.get("/api/user/userId", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ userId: decoded._id, username: user.username });
  } catch (error) {
    res.status(403).json({ message: "Invalid token" });
  }
});

// **Route sending otp to the user via mail**
...
app.post("/api/user/verify-otp", async (req, res) => {
  ...
  try {
    ...
    const now = new Date();
    if (now - otpRecord.createdAt > 10 * 60 * 1000) {
...
// **route that creates the otp and adding to Otp Model**
...
io.on("connection", async (socket) => {
  ...
  socket.on("fetchMessages", async (channelId) => {
    try {
      ...
      if (channel) socket.emit("channelMessages", channel.messages);
    } catch (error) {}
  });
